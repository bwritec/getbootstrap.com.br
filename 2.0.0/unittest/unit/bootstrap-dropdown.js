...
$(function ()
{
    ...
    test("deve adicionar classe aberta ao menu se clicado", function ()
    {
        var dropdownHTML = '<ul class="tabs">'
            ...
            + '</ul>'
            + '</li>'
            + '</ul>', dropdown = $(dropdownHTML).find('[data-toggle="dropdown"]').dropdown().click();

        ok(dropdown.parent('.dropdown').hasClass('open'), 'abrir classe adicionada ao clique');
    });
    ...
    test("deve remover a classe aberta se o body for clicado", function ()
    {
        ...
        ok(dropdown.parent('.dropdown').hasClass('open'), 'abrir classe adicionada ao clique.');
        $('body').click();
        ok(!dropdown.parent('.dropdown').hasClass('open'), 'abrir classe removida.');
        dropdown.remove();
    });
});
